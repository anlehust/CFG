...
public class Simulator {
    ...
    public static void main(String args[]) {
        ...
        readInputText(CFG_Text, "src/CFG_Example.txt");
        // Read input path file
        // File input has pattern: many lines, one line has:
        // NumberX "LabelXY" NumberY
        // No white space in the number, and no whitespace in the label
        readInputText(Path_Text, "src/Path_Example.txt");
...
//            endIdx = Integer.parseInt(stk.nextToken());
            ...
            for (int j=0; j<edges.size(); j++) {
                if (edges.get(j).GetStartString().equals(startIdx) && edges.get(j).GetEndString().equals(endIdx)) {
                    // Remove this edge 
                    edges.remove(j);
